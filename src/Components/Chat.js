...
const Chat = () => {
    ...
    useEffect(() => {
        setLoading(true)
        database.collection("chat").doc(`${chatId}`).get().then(res => {
            setUserId(
                res.data()?.users
            )
        })
    ...
    useEffect(() => {
        database.collection('chat').doc(`${chatId}`).collection('messages').orderBy("createdAt", "asc").onSnapshot(snapshot => {
    ...
    useEffect(() => {
        if (user && userId) {
            if (userId[0] === user.uid) {
                database.collection('users').doc(userId[1]).get().then(res => {
                    setOtherPreson(res.data())
                })
            } else {
                database.collection('users').doc(userId[0]).get().then(res => {
                    setOtherPreson(res.data())
                })
            }
        }
    ...
    const sendText = (e) => {
        e.preventDefault();
        (text !== '') &&
            database.collection('chat').doc(`${chatId}`).collection('messages').add({
                text: text,
                createdAt: new Date(),
                sender: user.uid
            }).then(
